...
class TestHeap(unittest.TestCase):
    ...
    def TestMinHeapProperty(self):
        input = [
            Node(label=0, adj_list=[], min_cost_edge=6),
            Node(label=1, adj_list=[], min_cost_edge=1),
            Node(label=2, adj_list=[], min_cost_edge=4),
            Node(label=3, adj_list=[], min_cost_edge=5),
            Node(label=4, adj_list=[], min_cost_edge=3),
            Node(label=5, adj_list=[], min_cost_edge=2),
        ]
        h = NodeHeap()
        for node in input:
            h.insert(node)
    ...
    def TestMinHeapWithRandomInput(self):
        input = [
            Node(label=i, adj_list=[], min_cost_edge=random.randrange(100))
        ...
        for i in range(len(input)):
            self.assertEqual(h.extract_min().min_cost_edge, sorted_input[i].min_cost_edge)
    ...
    def TestBuildMinHeap(self):
        ...
        for i in range(len(input)):
            print(h.m)
            self.assertEqual(h.peek_min().min_cost_edge, sorted_input[i].min_cost_edge)
            self.assertEqual(h.extract_min().min_cost_edge, sorted_input[i].min_cost_edge)
